...
# Vocabulary size constant (number of unique tile categories)
VOCAB_SIZE = 9
...
class ResActorCriticNetwork(nn.Module):
    def __init__(self, input_shape, num_action_types, block_type=ResConvBlockOneLayerFullyConnected, num_res_blocks=6):
        ...
        self.tile_embedding = nn.Embedding(VOCAB_SIZE, embedding_dim)
        linear = 256
        channels = [embedding_dim + 2, 64, 128, 256]
    ...
    def forward(self, grid, gold):
        # grid: (batch, H, W) of integer IDs
        # Embed grid IDs: (batch, H, W, embedding_dim)
        embedded = self.tile_embedding(grid.long())
        # Move channels to second dim: (batch, embedding_dim, H, W)
        embedded = embedded.permute(0, 3, 1, 2)
        # Expand gold to cover spatial dims
        gold_grid = gold.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, embedded.size(2), embedded.size(3))
        # Concatenate embedding channels and gold channels
        x = torch.cat((embedded, gold_grid), dim=1)
        ...
        output_grid = self.output_grid_head(shared_features)
